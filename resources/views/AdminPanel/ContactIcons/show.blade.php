...
@section('page_title')
    @lang('app.SHOW') @lang('app.CONTACT') @lang('app.ICONS')
@endsection

@section('content')
    <div class="card">
        <div class="card-header showCard-Header">
            <h6>@lang('app.SHOWING ALL') @lang('app.CONTACT') @lang('app.ICONS')</h6>
            {{-- @if (Auth::user()->hasPermission('users-create')) --}}
            <button type="button" class="btn btn-block btn-primary add_btn"><a class="formButton"
                    href="{{ url('/contactIcon/create') }}">@lang('app.ADD')</a></button>
            {{-- @else
                            ...
                            <tbody class="tbody_font">
                                @foreach ($showData as $singleData)
                                    <tr role="row" class="odd">
                                        ...
                                        <td>{{ $singleData->icon_image }}</td>
                                        <td>{{ $singleData->icon_link }}</td>
                                        <td class="option_ud col_width">
                                            {{-- @if (Auth::user()->hasPermission('users-delete')) --}}
                                            <a href="{{ route('contactIcon.show', $singleData->id) }}"
                                            ...
                                            <a href="{{ route('contactIcon.edit', ['contactIcon' => $singleData->id]) }}"
                                                type="submit"
                                                class="btn
