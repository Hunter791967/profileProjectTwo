...
@section('content')
    <div class="card">
        <div class="card-header showCard-Header">
            <h6>@lang('app.SHOWING ALL') @lang('app.GALLERY')</h6>
            {{-- @if (Auth::user()->hasPermission('users-create')) --}}
            <button type="button" class="btn btn-block btn-primary add_btn"><a class="formButton"
                    href="{{ url('/Gallery/create') }}">@lang('app.ADD') @lang('app.GALLERY')</a></button>
            {{-- @else
                                    ...
                                    <tr role="row" class="odd">
                                        ...
                                        <td class="disp_center">
                                            <img class="profile-user-img img-fluid img-circle usr_img"
                                                src="{{ asset('uploads/icons/' . $singleData->icon_image) }}"
                                                alt="User profile picture">
                                        </td>

                                        <td class="option_ud col_width">
                                            {{-- @if (Auth::user()->hasPermission('users-delete')) --}}
                                            <a href="{{ url("/Gallery/$singleData->id") }}"
                                            ...
                                            <a href="{{ route('Gallery.edit', ['Gallery' => $singleData->id]) }}"
                                                type="submit"
                                                class="btn
